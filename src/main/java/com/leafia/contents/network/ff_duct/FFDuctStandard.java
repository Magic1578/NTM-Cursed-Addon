...
public class FFDuctStandard extends FFDuctBase implements IDynamicModels, ILookOverlay, ICustomBlockItem, IBlockSpecialPlacementAABB {
	public static final PropertyInteger META = FluidDuctStandard.META;
	...
	private boolean canConnectTo(IBlockAccess world,int x,int y,int z,ForgeDirection dir,FluidType type) {
		...
		if (LeafiaUtil.canConnectFF(world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, dir, type)) {
			return true;
		}
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		if (te instanceof FFDuctTE pipe) {
	...
	public ItemStack getPickBlock(IBlockState state,RayTraceResult target,World world,BlockPos pos,EntityPlayer player) {
		...
		if (tileEntity instanceof FFDuctTE pipe) {
	...
	public void registerSprite(TextureMap map) {
		...
		String base0 = "blocks/fluid_duct_mk2";
		String overlay0 = base0 + "_overlay";
		baseSprites[0] = map.registerSprite(new ResourceLocation("hbm", base0));
		overlaySprites[0] = map.registerSprite(new ResourceLocation("hbm", overlay0));

		// meta 1: silver
		baseSprites[1] = map.registerSprite(new ResourceLocation("leafia", "models/leafia/lftr/pipe_silver"));
		overlaySprites[1] = map.registerSprite(new ResourceLocation("leafia", "models/leafia/lftr/pipe_silver_overlay"));

		// meta 2: colored
		baseSprites[2] = map.registerSprite(new ResourceLocation("hbm", "blocks/pipe_colored"));
		overlaySprites[2] = map.registerSprite(new ResourceLocation("leafia", "models/leafia/lftr/pipe_colored_overlay"));
	}
	...
	public static void registerColorHandler(ColorHandlerEvent.Block evt) {
		IBlockColor colorHandler = (state,worldIn,pos,tintIndex) -> {
			...
			if (!(te instanceof FFDuctTE pipe)) return 0xFFFFFF;
			FluidType type = pipe.getType();
			if (type == null) return 0xFFFFFF;
			return type.getColor();
		};
		evt.getBlockColors().registerBlockColorHandler(colorHandler, AddonBlocks.ff_duct); // ensure ModBlocks.fluid_duct refers to this block
	}
}
